...
class NoteWindow:
    def __init__(self, title="Main Title", theme="aqua", layout_type="pop_layout"):
        ...
        self.pop_layout = [[sg.Text('Persistent window')],
                           [sg.Input(key='-IN-')],
                           [sg.Button('Read'), sg.Exit()]]
        self.draw_layout = [[sg.Canvas(background_color="#ffffff", size=(200, 200), pad=(5, 5),
                                       key="draw_pad", tooltip="Draw here", visible=True, border_width=3)],
                            [sg.Text('Drawing Window')],
                            [sg.Button('Read', button_color=("#000000", "#991a1a")),
                             sg.Exit(button_color=("#000000", "#991a1a"))]]
        self.note = sg.Window(self.title_of_main, self.layouts[layout_type], ttk_theme=self.theme).finalize()

    def set_cursor(self, which_element='draw_pad', cursor_type='pencil'):
        self.note[which_element].set_cursor(cursor_type, '#ff0000')

    def note_go(self):
        if self.layout_type == 'draw_layout':
            self.set_cursor()
            self.note["draw_pad"].bind("<Button-1>", self.save_posn)
            self.note["draw_pad"].bind("<B1-Motion>", self.draw_shape)
    ...
    def draw_shape(self, event, which_element='draw_pad'):
    ...
    def clear_canvas(self, which_element='draw_pad'):
